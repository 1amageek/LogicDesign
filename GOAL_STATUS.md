...
| Contract test | Passed | timeout-bounded `xcodebuild test`; 70 tests in 7 suites |
...
| Dataflow/process IR | Contract defined | Typed functions, SSA nodes, arbitrary-width values, process state/next-state, channels, flow/FIFO policy, external-independent arithmetic/bitwise/comparison/shift operations and validation | Canonical success/failure tests plus XLS interoperability corpus | Complete for declared canonical operation subset |
